...
exports.MultiTargetTracker = void 0;
const ukf_1 = require("../ukf");
// ============================================================================
// Tracker
// ============================================================================
class MultiTargetTracker {
    constructor(dt = 0.1, Q, R) {
        this.tracks = new Map();
        this.nextId = 1;
        this.idSwitches = 0;
        this.birthThreshold = 2; // consecutive unassociated measurements
        this.deathThreshold = 8; // steps without association
        this.dt = dt;
        this.Q = Q || (0, ukf_1.defaultProcessNoise)(dt);
        this.R = R || (0, ukf_1.defaultMeasurementNoise)();
    }
    /**
     * Process a set of measurements for the current step.
     * Returns assignment results and updated tracks.
     */
    step(measurements, currentStep) {
        ...
        for (const a of assignment.assignments) {
            ...
            if (track) {
                track.state = (0, ukf_1.ukfStep)(track.state, a.measurement, this.dt, this.Q, this.R);
     ...
     */
    associate(tracks, measurements, currentStep) {
        const assignments = [];
        const assignedTracks = new Set();
        const assignedMeasurements = new Set();
        ...
        for (const pair of pairs) {
            if (assignedTracks.has(pair.trackIdx) || assignedMeasurements.has(pair.measIdx))
                continue;
            // Gate: only assign if within reasonable Mahalanobis distance
            // χ²(3, 95%) = 7.815
            if (pair.distance > 20.0)
                continue; // generous gate for tracking
     ...
     */
    mahalanobisDistance(state, measurement) {
        const innov = [
     ...
     */
    handleUnassignedMeasurement(measurement, currentStep) {
        ...
        for (const track of this.tracks.values()) {
            if (!track.alive && track.pendingMeasurements.length > 0) {
                ...
                const dist = Math.sqrt((measurement[0] - last[0]) ** 2 +
...
exports.MultiTargetTracker = MultiTargetTracker;
