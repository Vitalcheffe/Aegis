...
describe('UKF configuration', () => {
    test('lambda computed correctly for L=9, α=1e-3, κ=0', () => {
        const lambda = (0, ukf_1.computeLambda)(9, 1e-3, 0);
        // λ = α²(L+κ) - L = (1e-6)(9) - 9 = 9e-6 - 9 ≈ -9
        expect(lambda).toBeCloseTo(9e-6 - 9, 6);
    });
    test('config matches AEGIS_UKF_MATH.md spec', () => {
        expect(ukf_1.UKF_CONFIG.L).toBe(9);
        expect(ukf_1.UKF_CONFIG.alpha).toBeCloseTo(1e-3, 10);
        expect(ukf_1.UKF_CONFIG.beta).toBe(2);
        expect(ukf_1.UKF_CONFIG.kappa).toBe(0);
    });
});
describe('Sigma point generation', () => {
    test('generates 2L+1 = 19 sigma points for L=9', () => {
        const x = new Array(9).fill(0);
        const P = (0, ukf_1.identity)(9);
        const { points } = (0, ukf_1.generateSigmaPoints)(x, P);
    ...
    test('sigma point weights sum to 1 (mean)', () => {
        const x = new Array(9).fill(0);
        const P = (0, ukf_1.identity)(9);
        const { Wm } = (0, ukf_1.generateSigmaPoints)(x, P);
    ...
    test('sigma point weighted mean equals original mean', () => {
        const x = [1, 2, 3, 4, 5, 6, 7, 8, 9];
        const P = (0, ukf_1.matscale)((0, ukf_1.identity)(9), 2.0);
        const { points, Wm } = (0, ukf_1.generateSigmaPoints)(x, P);
...
describe('State transition function', () => {
    test('constant velocity with zero acceleration', () => {
        ...
        const dt = 1;
        const newX = (0, ukf_1.stateTransition)(x, dt);
...
describe('Measurement function', () => {
    test('returns first 3 components (position only)', () => {
        const x = [1, 2, 3, 4, 5, 6, 7, 8, 9];
        const z = (0, ukf_1.measurementFunction)(x);
...
describe('UKF predict step', () => {
    test('predicts forward in time without NaN', () => {
        ...
        const Q = (0, ukf_1.defaultProcessNoise)(0.1);
        const predicted = (0, ukf_1.ukfPredict)(state, 0.1, Q);
    ...
    test('predict increases covariance (uncertainty grows)', () => {
        const state = { x: new Array(9).fill(0), P: (0, ukf_1.matscale)((0, ukf_1.identity)(9), 0.1) };
        const Q = (0, ukf_1.defaultProcessNoise)(0.1);
        const predicted = (0, ukf_1.ukfPredict)(state, 0.1, Q);
...
describe('UKF update step', () => {
    test('filter converges toward truth over 50 steps', () => {
        ...
        let filterState = (0, ukf_1.initialState)();
        const Q = (0, ukf_1.defaultProcessNoise)(dt);
        const R = (0, ukf_1.defaultMeasurementNoise)();
        let finalError = Infinity;
        for (let step = 0; step < 50; step++) {
            // Advance truth
            trueState = (0, ukf_1.stateTransition)(trueState, dt);
            // Generate noisy measurement
            const z = (0, ukf_1.measurementFunction)(trueState).map((v) => v + (Math.random() - 0.5) * 2);
            // Filter step
            filterState = (0, ukf_1.ukfStep)(filterState, z, dt, Q, R);
            // Track error
            const posError = Math.sqrt(Math.pow(filterState.x[0] - trueState[0], 2) +
    ...
    test('filter beats raw measurement noise', () => {
        ...
        for (let step = 0; step < 100; step++) {
            ...
            if (step > 10) { // skip warmup
                const filterErr = Math.sqrt(Math.pow(filterState.x[0] - trueState[0], 2) +
                    Math.pow(filterState.x[1] - trueState[1], 2) +
                    Math.pow(filterState.x[2] - trueState[2], 2));
                const measErr = Math.sqrt(Math.pow(z[0] - trueState[0], 2) +
