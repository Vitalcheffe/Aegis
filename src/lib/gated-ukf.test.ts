import { gatedUKFStep, runGatedUKF } from './gated-ukf';
import { initialState, defaultProcessNoise, defaultMeasurementNoise } from './ukf';
...
describe('Gated UKF — Outlier Robustness (A16)', () => {
  ...
  function generateTrajectory(steps: number, outlierRate: number, rng: () => number) {
  ...
  function computeTruePosition(step: number) {
    const t = step * dt;
    return [
      8 * Math.sin(0.4 * t),
      4 + 2 * Math.cos(0.4 * t),
  ...
  test('5% outliers at 20m, 200 steps, 10 runs → 0 divergence', () => {
    ...
    for (let run = 0; run < runs; run++) {
      ...
      for (let s = 0; s < steps; s++) {
        const result = gatedUKFStep(state, measurements[s], dt, Q, R);
        state = result.state;
        
        const truePos = computeTruePosition(s);
        
        if (s >= 50) {  // warmup
          const fErr = Math.sqrt(
            (state.x[0] - truePos[0])**2 +
            (state.x[1] - truePos[1])**2 +
            (state.x[2] - truePos[2])**2
          );
          filterErrors.push(fErr);
          
          const mErr = Math.sqrt(
            (measurements[s][0] - truePos[0])**2 +
      ...
      if (maxError > 10.0) totalDiverged++;
    }
  ...
  test('gate rejects measurements with NIS > threshold', () => {
    ...
    const result1 = gatedUKFStep(state, z1, dt, Q, R);
    expect(result1.accepted).toBe(true);
    
    // Outlier measurement (20m away)
    const z2 = [20, -15, 25];
    // Need to use the state after step 1
    const result2 = gatedUKFStep(result1.state, z2, dt, Q, R);
  ...
  test('after rejection, filter continues with good measurements', () => {
    ...
    // Step 1: normal
    state = gatedUKFStep(state, [0.1, 4.1, 0.1], dt, Q, R).state;
    
    // Step 2: outlier (should be rejected)
    const result2 = gatedUKFStep(state, [50, 50, 50], dt, Q, R);
    expect(result2.accepted).toBe(false);
    state = result2.state;
    
    // Step 3: normal again (should be accepted)
    const result3 = gatedUKFStep(state, [0.2, 4.0, 0.2], dt, Q, R);
  ...
  test('no outliers → 0 rejections', () => {
    ...
    const result = runGatedUKF(initialState(), measurements, dt, Q, R);
