import {
  positionMeasurement, bearingMeasurement, rangeRateMeasurement, combinedMeasurement,
  positionR, bearingR, combinedR,
} from './measurements';

describe('Position Measurement', () => {
  test('returns first 3 components', () => {
    expect(positionMeasurement([5, 6, 7, 1, 0, 0, 0, 0, 0])).toEqual([5, 6, 7]);
  });
});
...
describe('Range-Rate Measurement', () => {
  test('approaching target (vx < 0, x > 0) has negative range-rate', () => {
    ...
    const rr = rangeRateMeasurement(x)[0];
  ...
  test('range-rate at origin is 0 (protected)', () => {
    ...
    expect(rangeRateMeasurement(x)[0]).toBe(0);
  });
...
describe('Combined Measurement (5D)', () => {
  ...
  test('bearing and range-rate consistent with individual functions', () => {
    ...
    const combined = combinedMeasurement(x);
    const bearing = bearingMeasurement(x)[0];
    const rr = rangeRateMeasurement(x)[0];
