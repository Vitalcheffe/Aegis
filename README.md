...
```
SensorFrames × N  (EO + LWIR + Doppler · 120 bytes/frame · 50 Hz)
        │
        ▼
┌───────────────────────────────────────┐
│  M1  SpectralFusion                   │  MAD Byzantine filter
│      fusion/spectral_fusion.py        │  5-rule lure detector
│                                       │  O(N) · < 1ms for 500 drones
└───────────────────┬───────────────────┘
                    │ List[FusedTarget]
                    ▼
┌───────────────────────────────────────┐
│  M2  MultiTargetUKF                   │  9-state UKF [px,py,pz,vx,vy,vz,ax,ay,az]
│      ukf/intercept_ukf.py             │  Intercept scan: 120 windows × 0.1s
│                                       │  Vectorized σ-point propagation · 0.9ms
└───────────────────┬───────────────────┘
                    │ refined FusedTargets
                    ▼
┌───────────────────────────────────────┐
│  M3  ElasticNet                       │  Spring-physics formation
│      swarm/elastic_net.py             │  6-neighbour graph O(6N) — 83× vs O(N²)
│                                       │  FSM: PATROL → ENGAGE → PURSUIT → REFORM
└───────────────────┬───────────────────┘
                    │ formation commands
                    ▼
┌───────────────────────────────────────┐
│  M4  EnergyBudget                     │  3-reserve model (combat / RTB / chute)
│      energy/budget_manager.py         │  TargetWorthEngine: threat score [0–100]
└───────────────────┬───────────────────┘
                    │ energy decisions
                    ▼
┌───────────────────────────────────────┐
│  Safety  ProximityLock                │  Default: LOCKED · 4-check fail-safe
│          HumanLoopGate                │  Colonel veto window: 8s
│          ADSBSpoofDetector            │  Kinematic consistency check
└───────────────────┬───────────────────┘
                    │
                    ▼
        DroneCommands × N      (50 Hz · 48 bytes/drone)
```
...
```
src/aegis/
├── oc_types.py              # Shared types, physical constants, self-test (1976 lines)
├── origin_core.py           # 50 Hz orchestrator — wires all modules
├── fusion/
│   └── spectral_fusion.py  # M1: MAD Byzantine filter + lure detector
├── ukf/
│   └── intercept_ukf.py    # M2: 9-state UKF + vectorized intercept scan
├── swarm/
│   └── elastic_net.py      # M3: Spring-physics formation O(6N)
├── energy/
│   └── budget_manager.py   # M4: 3-reserve energy model + TargetWorthEngine
└── safety/
    └── proximity_lock.py   # ProximityLock + HumanLoopGate + ADS-B spoof detector

tests/test_all.py            # 63 unit tests
simulations/nevada_scenario.py  # 3-wave engagement simulation
docs/
├── BOM.md                  # Full Bill of Materials with prices
└── figures/                # Generated plots from real code
```
